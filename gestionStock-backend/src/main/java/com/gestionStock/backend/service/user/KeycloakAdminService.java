...
public class KeycloakAdminService {
	...
	public List<Map<String, Object>> getAllUsers() {
		return executeWithRetry(() -> webClient.get().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users")
				.headers(h -> h.setBearerAuth(getAdminToken())).retrieve()
	...
	public String createUser(Map<String, Object> user) {
		return executeWithRetry(() -> {
			var response = webClient.post().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users")
					.headers(h -> h.setBearerAuth(getAdminToken())).bodyValue(user).retrieve()
	...
	public Map<String, Object> getUserByEmail(String email) {
		return executeWithRetry(() -> {
			...
			List<Map<String, Object>> users = webClient.get().uri(url).headers(h -> h.setBearerAuth(getAdminToken()))
					.retrieve()
					.bodyToMono(new org.springframework.core.ParameterizedTypeReference<List<Map<String, Object>>>() {
					}).block();
	...
	public void deleteUser(String id) {
		executeWithRetry(() -> {
			webClient.delete().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users/" + id)
					.headers(h -> h.setBearerAuth(getAdminToken())).retrieve().bodyToMono(Void.class).block();
			return null;
		});
	}
	...
	public void updateUserEnabledStatus(String id, boolean enabled) {
		executeWithRetry(() -> {
			...
			webClient.put().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users/" + id)
					.headers(h -> h.setBearerAuth(getAdminToken())).bodyValue(update).retrieve().bodyToMono(Void.class)
					.block();
			return null;
		});
	}
	...
	public void resetUserPassword(String id, String newPassword) {
		executeWithRetry(() -> {
			...
			webClient.put().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users/" + id + "/reset-password")
					.headers(h -> h.setBearerAuth(getAdminToken())).bodyValue(passwordCred).retrieve()
	...
	public void assignRole(String userId, String roleName) {
		...
		webClient.post()
				.uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/users/" + userId + "/role-mappings/realm")
				.headers(h -> h.setBearerAuth(getAdminToken())).bodyValue(List.of(role)).retrieve()
				.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
	...
	public Map<String, Object> getRealmRole(String roleName) {
		return executeWithRetry(() -> {
			try {
				return webClient.get().uri(keycloakServerUrl + "/admin/realms/" + targetRealm + "/roles/" + roleName)
						.headers(h -> h.setBearerAuth(getAdminToken())).retrieve()
